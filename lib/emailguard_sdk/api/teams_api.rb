...
module Emailguard
  class TeamsApi
    ...
    # @param [Hash] opts the optional parameters
    # @return [GetPublicTeamResponse]
    def update_team(update_team_input, opts = {})
      data, _status_code, _headers = update_team_with_http_info(update_team_input, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @return [Array<(GetPublicTeamResponse, Integer, Hash)>] GetPublicTeamResponse data, response status code and response headers
    def update_team_with_http_info(update_team_input, opts = {})
      if @api_client.config.debugging
      ...
      if @api_client.config.client_side_validation && update_team_input.nil?
        fail ArgumentError, "Missing the required parameter 'update_team_input' when calling TeamsApi.update_team"
      end
      # resource path
      local_var_path = '/api/v1/team'
